...
public class CacheBackedCORSOriginDAO extends CORSOriginDAOImpl {
    ...
    private void addCORSOriginsToCacheOnRead(CORSOrigin[] corsOrigins, int tenantId) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("[AddToCacheOnRead] Adding CORS origins to Cache with Key: " + tenantId);
        }

        CORSOriginCache.getInstance().addToCacheOnRead(cacheKey, cacheEntry);
    }
    ...
    private void addCORSOriginsToCacheOnRead(CORSOrigin[] corsOrigins, int applicationId, int tenantId) {
        ...
        CORSOriginByAppIdCacheEntry cacheEntry = new CORSOriginByAppIdCacheEntry(corsOrigins);
        if (log.isDebugEnabled()) {
            log.debug("[AddToCacheOnRead] Adding CORS origins to Cache for application id: " + applicationId +
                    ", tenant id: " + tenantId);
        }

        CORSOriginByAppIdCache.getInstance().addToCacheOnRead(cacheKey, cacheEntry);
    }
