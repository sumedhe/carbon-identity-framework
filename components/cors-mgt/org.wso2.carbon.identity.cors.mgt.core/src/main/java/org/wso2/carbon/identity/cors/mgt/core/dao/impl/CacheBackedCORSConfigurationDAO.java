...
public class CacheBackedCORSConfigurationDAO extends CORSConfigurationDAOImpl {
    ...
    private void addCORSConfigurationToCacheOnRead(CORSConfiguration corsConfiguration, String tenantDomain) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("[AddToCacheOnRead] Adding CORS configuration to Cache with Key: " + tenantDomain);
        }

        CORSConfigurationCache.getInstance().addToCacheOnRead(cacheKey, cacheEntry);
    }
