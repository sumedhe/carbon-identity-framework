...
public class CachedBackedConfigurationDAO implements ConfigurationDAO {
    ...
    private void addResourceToCacheOnRead(Resource resource) throws ConfigurationManagementException {
        ...
        ResourceCacheEntry resourceCacheEntry = new ResourceCacheEntry(resource);
        if (log.isDebugEnabled()) {
            String message = String.format("[AddToCacheOnRead] Following two cache entries created. 1. Resource by " +
                            "name cache %s, 2. Resource by id cache %s. Tenant domain for all caches: %s",
                    resource.getResourceName(), resource.getResourceId(), resource.getTenantDomain());
            log.debug(message);
        }
